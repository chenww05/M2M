...
void addText(Mat*dest, int height, int width) {
    ...
    text = "Estimated Shoulder Width is: " + to_string(width) + " cm";
...
void showImg(BodyModel* model, Mat* src, int iteration)
{
    ...
    int adjusted_height = angle_anjust->getHeight(upper_pixel, lower_pixel);
    int adjusted_width = angle_anjust->getShoulderWidth(model->_right_shoulder.y, model->_right_shoulder.x - model->_center_x);
    addText(&dest, adjusted_height, adjusted_width);
